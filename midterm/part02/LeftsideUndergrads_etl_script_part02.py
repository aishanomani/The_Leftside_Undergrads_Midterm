...
def extract_sales_from_shards():
    """Extract customers, products, and orders from both sales shards and merge results."""
    
    se_conn = get_connection("SALES_SHARD_SE")
    ne_conn = get_connection("SALES_SHARD_NE")

    customers_se, products_se, orders_se = extract_sales(se_conn)
    customers_ne, products_ne, orders_ne = extract_sales(ne_conn)
    ...
    products = products_se + products_ne
    orders = orders_se + orders_ne

    # Remove duplicates
    customers = list({c[0]: c for c in customers}.values())
    products = list({p[0]: p for p in products}.values())

    print(f"  Total merged customers from shards: {len(customers)}")
    print(f"  Total merged products from shards: {len(products)}")
    print(f"  Total merged orders from shards: {len(orders)}")
...
def run_etl():
    ...
    customers, products, orders = extract_sales_from_shards()
